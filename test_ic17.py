...
from dataset import IC17TestLoader
import models
import util
...
def test(args):
    ...
    for idx, (org_img, img) in enumerate(test_loader):
        ...
        debug(idx, data_loader.img_paths, [[text_box,score_s],[link_score,mask]], 'outputs/vis_ic17/')

    return
    cmd = 'cd %s;zip -j %s %s/*'%('./outputs/', 'submit_ic17.zip', 'submit_ic17');
    print(cmd)
    sys.stdout.flush()
    util.cmd.cmd(cmd)
